...
class MultiHeadLatentAttention(nn.Module):
    def __init__(
        ...
        self.kv_down = nn.Linear(self.dim, lora_rank, bias=False)
        self.kv_norm_w = nn.Parameter(torch.ones(lora_rank))
        self.kv_up_k_nope = nn.Linear(lora_rank, self.heads * self.nope_dim, bias=False)
        self.k_rope = nn.Linear(self.dim, self.rope_dim, bias=False)
        self.kv_up_v = nn.Linear(lora_rank, self.heads * self.head_dim, bias=False)

        self.out_proj = nn.Linear(dim, dim, bias=False)

        inv_freq = 1.0 / (12800.0 ** (torch.arange(0, self.rope_dim, 2).float() / self.rope_dim))
        t = torch.arange(max_seq_len)
        freqs = torch.outer(t, inv_freq)
        emb = torch.cat([freqs, freqs], dim=-1)

        self.register_buffer("cos_cache", emb.cos().unsqueeze(0).unsqueeze(2), persistent=False)
        self.register_buffer("sin_cache", emb.sin().unsqueeze(0).unsqueeze(2), persistent=False)
    ...
    def forward(self, x, offset=0, kv_cache=None):
        ...
        q_rope = q_rope * cos + self._rotate_half(q_rope) * sin
        q_full = torch.cat([q_nope, q_rope], dim=-1)

        kv_latent = F.linear(x, self.kv_down.weight)
        kv_latent = F.rms_norm(kv_latent, (kv_latent.shape[-1],), self.kv_norm_w, 1e-6)

        k_rope_new = self.k_rope(x).view(B, T, 1, Dr)

        if kv_cache is not None and not self.training:
            lat_prev, k_rope_prev = kv_cache
            kv_latent = torch.cat([lat_prev, kv_latent], dim=1)
            k_rope_raw = torch.cat([k_rope_prev, k_rope_new], dim=1)
            T_full = kv_latent.shape[1]
        else:
            k_rope_raw = k_rope_new
            T_full = T

        cos_full = self.cos_cache[:, offset : offset + T_full, :, :]
        sin_full = self.sin_cache[:, offset : offset + T_full, :, :]
        k_rope_rotated = k_rope_raw * cos_full + self._rotate_half(k_rope_raw) * sin_full
        k_rope_full = k_rope_rotated.expand(-1, -1, H, -1)

        k_nope_full = self.kv_up_k_nope(kv_latent).view(B, T_full, H, Dn)
        v_full = self.kv_up_v(kv_latent).view(B, T_full, H, self.head_dim)

        k_full = torch.cat([k_nope_full, k_rope_full], dim=-1)
        del k_nope_full, k_rope_full

        q_attn = q_full.permute(0, 2, 1, 3).contiguous()
        k_attn = k_full.permute(0, 2, 1, 3).contiguous()
        v_attn = v_full.permute(0, 2, 1, 3).contiguous()
        ...
        new_cache = (kv_latent, k_rope_raw) if not self.training else None
